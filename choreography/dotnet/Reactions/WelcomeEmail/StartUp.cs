...
public class StartUp : BenzeneStartUp
{
    ...
    public override void ConfigureServices(IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.UsingBenzene(x => x
            .AddCorrelationId()
            .AddMessageHandlers(typeof(SendWelcomeEmailOnTenantCreated).Assembly)
    ...
    public override void Configure(IBenzeneApplicationBuilder app, IConfiguration configuration)
    {
        ...
        app.UseWorker(workers => workers.UseRabbitMq(config, connectionFactory,
            rabbit => rabbit
                ...
                {
                    // Six lines that Benzene does not ship a counterpart for. Benzene.Clients stamps
                    // the correlation id on the way OUT (UseCorrelationId), and the inbound side reads
                    // it onto the diagnostics span - but nothing puts it back into ICorrelationId, so
                    // without this a reaction's own correlation id is a fresh Guid and the chain is
                    // broken exactly where a reader would look for it.
